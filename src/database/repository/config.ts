...
export class Config {
  async set(key: string, value: string) {
    const result = await prisma.admin_config.upsert({
      where: { key },
      create: { key, value },
      update: { key, value },
    });
  ...
  async get(key: string) {
    const result = await prisma.admin_config.findUnique({ where: { key } });
    if (result)
      return {
        key: result.key,
        value: result.value,
  ...
  public async getMany(keys: string[]) {
    ...
    for (let i = 0; i < keys.length; i++) {
      const data = await prisma.admin_config.findUnique({ where: { key: keys[i] } });
      if (data) {
